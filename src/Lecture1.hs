...
-}
strSum :: [Char] -> Int
strSum str = count 0 (words str) 
    where
        count :: Int -> [String] -> Int
        count result nums
            | null nums = result
            | otherwise = count (result + read (head nums)) (tail nums)
...
lowerAndGreater n = go 0 0
    where
        ...
        go nLess nMore l
            | null l = show n ++ " is greater than " ++ show nLess ++ " elements and lower than " ++ show nMore ++ " elements"
            | otherwise =
                if head l < n 
                then go (nLess + 1) nMore (tail l)
                else
                    if head l > n
                    then go nLess (nMore + 1) (tail l)
                    else go nLess nMore (tail l)
